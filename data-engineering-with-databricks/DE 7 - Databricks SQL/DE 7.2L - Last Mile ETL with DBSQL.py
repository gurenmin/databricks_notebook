...
# MAGIC %sql
# MAGIC CREATE DATABASE IF NOT EXISTS brian_smida_onvc_da_delp
# MAGIC LOCATION 'dbfs:/mnt/dbacademy-users/brian.smida@publicismedia.com/data-engineer-learning-path';
# MAGIC
# MAGIC USE brian_smida_onvc_da_delp;
# MAGIC
...
# MAGIC USE brian_smida_onvc_da_delp;
# MAGIC
# MAGIC SELECT user_id, count(*) total_records
# MAGIC FROM user_ping
# MAGIC GROUP BY user_id
# MAGIC ORDER BY 
# MAGIC   total_records DESC,
# MAGIC   user_id ASC;
...
# MAGIC USE brian_smida_onvc_da_delp;
# MAGIC
# MAGIC SELECT user_id, window.end end_time, mean(ping) avg_ping
# MAGIC FROM user_ping
# MAGIC GROUP BY user_id, window(time, '3 minutes')
# MAGIC ORDER BY
# MAGIC   end_time DESC,
# MAGIC   user_id ASC;
...
# MAGIC USE brian_smida_onvc_da_delp;
# MAGIC
# MAGIC SELECT user_id, min(time) first_seen, max(time) last_seen, count(*) total_records, avg(ping) total_avg_ping
# MAGIC FROM user_ping
# MAGIC GROUP BY user_id
# MAGIC ORDER BY user_id ASC;
